-- This script was generated by gen_SQL_cmd.py for organizing osm tables.
-- Run the following in psql in order to set up OSM tables.
        ...
        CREATE TABLE line_features(
            ...
            SELECT nextval('line_features_gid_seq'::regclass), 
                   t1.osm_id, 
                   'water' AS geo_feature, 
                   t1.water AS feature_type, 
                   t1.wkb_geometry
            FROM lines t1, 
                 (SELECT t.water, count(t.water) as count 
                  FROM lines t WHERE t.water IS NOT NULL GROUP BY t.water) t2
            WHERE t1.water is not NULL and ST_IsValid(t1.wkb_geometry) 
                    and t1.water = t2.water and t2.count > 30
                    and t1.water != 'yes';
        ...
        CREATE TABLE polygon_features(
            ...
            SELECT nextval('polygon_features_gid_seq'::regclass), 
                   t1.osm_id, 
                   'geological' AS geo_feature, 
                   t1.geological AS feature_type, 
                   t1.wkb_geometry
            FROM multipolygons t1, 
                 (SELECT t.geological, count(t.geological) as count 
                  FROM multipolygons t WHERE t.geological IS NOT NULL GROUP BY t.geological) t2
            WHERE t1.geological is not NULL and ST_IsValid(t1.wkb_geometry) 
                    and t1.geological = t2.geological and t2.count > 30
                    and t1.geological != 'yes';
            ...
            FROM multipolygons t1, 
                 (SELECT t.water, count(t.water) as count 
                  FROM multipolygons t WHERE t.water IS NOT NULL GROUP BY t.water) t2
            WHERE t1.water is not NULL and ST_IsValid(t1.wkb_geometry) 
                    and t1.water = t2.water and t2.count > 30
                    and t1.water != 'yes';
